...
def test(dataLoader, netmodel, optimizer, epoch, exp_args):
    ...
    for i, (input_ori, input, edge, mask) in enumerate(dataLoader):  
        ...
        # input_ori_var = Variable(input_ori)
        # input_var = Variable(input)
        # edge_var = Variable(edge)
...
def train(dataLoader, netmodel, optimizer, epoch, exp_args):
    ...
    for i, (input_ori, input, edge, mask) in enumerate(dataLoader):  
        ...
        if i % (args.printfreq) == 0:
            ...
            print(f"avg pred: {np.mean(masks_pred)} max pred: {np.max(masks_pred)} min pred: {np.min(masks_pred)}")
...
def main(args):
    ...
    netmodel = PotraitNet(useUpsample=exp_args.useUpsample, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', 
                        default='PotraitNet\config\model_mobilenetv2_without_auxiliary_losses.yaml', 
